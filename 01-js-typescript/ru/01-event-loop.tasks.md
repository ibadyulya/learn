...
> 🌐 English version: [01-event-loop.tasks.md](../en/01-event-loop.tasks.md)

Для каждой задачи предскажи **порядок вывода**, не запуская код. Где порядок
недетерминирован — так и напиши и объясни почему. Разборы — в [01-event-loop.answers.md](./01-event-loop.answers.md).
